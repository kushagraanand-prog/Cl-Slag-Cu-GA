...
# Helper: evaluate candidate (in original unscaled feature space)
def candidate_fitness_func(candidate):
    # candidate is 1D array length = n_features
    # get candidate array, convert to shape(1,-1)
    cand = np.array(candidate, dtype=float).reshape(1, -1)
    # enforce rounding for integer features
    for f in integer_features:
        idx = feat_index[f]
        cand[0, idx] = round(cand[0, idx])
    # scale candidate and get prob
    try:
        cand_scaled = scaler.transform(cand)
    except Exception as e:
        return -10.0  # invalid candidate
    prob_vec = model.predict_proba(cand_scaled)[0]
    prob_target = float(prob_vec[desired_class])
    # distance (normalized by feature range)
    dist = 0.0
    for i, f in enumerate(FEATURE_ORDER):
        lo, hi = permitted_range[f]
        rng = hi - lo if hi - lo > 0 else 1.0
        dist += ((cand[0,i] - orig[i])**2) / (rng**2)
    dist = np.sqrt(dist)
    # fitness: higher is better
    fitness = prob_target - LAMBDA * dist
    # small penalty if candidate violates permitted_range (shouldn't happen if gene_space correct)
    for i, f in enumerate(FEATURE_ORDER):
        lo, hi = permitted_range[f]
        if cand[0,i] < lo - 1e-8 or cand[0,i] > hi + 1e-8:
            fitness -= 1.0
